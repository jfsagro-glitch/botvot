...
- `DRIVE_MASTER_DOC_ID=<id документа>` *(берётся из ссылки вида `https://docs.google.com/document/d/<id>/edit` )*
  - **Текущий документ:** `1K6GdcvclPILrwuVTNdbXbmBSwWd5bSbTRK1T0PATu70`
  - **Ссылка:** https://docs.google.com/document/d/1K6GdcvclPILrwuVTNdbXbmBSwWd5bSbTRK1T0PATu70/edit
...
### Откат обновлений
В админ-боте (ПУП) доступна кнопка **"⏪ Откатить обновление"** для возврата к предыдущей версии уроков.
Бэкапы создаются автоматически при каждой синхронизации и хранятся в `content_backups/`.
