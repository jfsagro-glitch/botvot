"""
Скрипт для загрузки всех медиа файлов в Telegram и получения file_id.

Загружает все медиа из Photo/video_pic в Telegram бота,
получает file_id и обновляет lessons.json с этими file_id.
Это оптимизирует отправку - file_id работает намного быстрее, чем загрузка с диска.
"""
...
async def upload_media_file(bot: Bot, file_path: Path, media_type: str, test_chat_id: int, max_retries: int = 3) -> Optional[str]:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            timeout = 300 if file_size > 10 else 120  # 5 минут для больших, 2 минуты для маленьких
            ...
            if attempt < max_retries:
                wait_time = attempt * 5  # Увеличиваем время ожидания с каждой попыткой
                print(f"   ⏳ Таймаут (попытка {attempt}/{max_retries}), ждем {wait_time} сек...")
                await asyncio.sleep(wait_time)
                continue
            else:
                print(f"   ❌ Таймаут после {max_retries} попыток")
                return None
            ...
            if "timeout" in error_msg.lower() or "Connection" in error_msg:
                if attempt < max_retries:
                    wait_time = attempt * 5
                    print(f"   ⏳ Ошибка сети (попытка {attempt}/{max_retries}), ждем {wait_time} сек...")
                    await asyncio.sleep(wait_time)
                    continue
                else:
                    print(f"   ❌ Ошибка сети после {max_retries} попыток: {e}")
                    return None
            else:
...
async def process_media_files(bot: Bot, media_dir: Path, test_chat_id: int) -> Dict[int, List[Dict[str, str]]]:
    ...
    for file_path in all_files:
        ...
        if file_id:
            ...
            # Формируем относительный путь для совместимости
            ...
            print(f"   ❌ Не удалось получить file_id после всех попыток")
            # Сохраняем путь без file_id для fallback
            if lesson_num not in media_by_lesson:
                media_by_lesson[lesson_num] = []
            relative_path = f"Photo/video_pic/{filename}"
            media_by_lesson[lesson_num].append({
        ...
        # Увеличиваем паузу между загрузками для стабильности
...
def update_lessons_with_file_ids(lessons_file: Path, media_by_lesson: Dict[int, List[Dict[str, str]]]):
    ...
    for lesson_num, media_list in media_by_lesson.items():
        ...
        updated_count += 1
        print(f"✅ Обновлен урок {lesson_num} с {len(media_list)} медиа файлами (file_id сохранены)")
    
    # Сохраняем обновленный lessons.json
    if updated_count > 0:
        # Создаем резервную копию
        backup_file = lessons_file.with_suffix('.json.backup_media')
...
async def main():
    ...
    # ID чата для тестовой отправки
    test_chat_id = Config.ADMIN_CHAT_ID
    if not test_chat_id:
        print("⚠️  ADMIN_CHAT_ID не настроен.")
        print("   Чтобы узнать свой ID, напишите боту @userinfobot")
        test_chat_id_input = input("Введите ваш Telegram ID (число): ").strip()
        try:
            test_chat_id = int(test_chat_id_input)
        except ValueError:
            print("❌ Неверный ID!")
            return
    
    print(f"📱 Используется чат ID: {test_chat_id}")
    print(f"📂 Медиа директория: {media_dir}")
    print()
    ...
    try:
        ...
        print()
        print("⚠️  ВАЖНО: Удалите загруженные медиа из чата с ботом,")
        print("   чтобы не засорять историю сообщений.")
