...
class SalesBot:
    ...
    async def _show_program_tariff_menu(self, message: Message):
        ...
        text = (
            "👋 <b>Привет!</b> Я помогу тебе записаться на курс по искусству задавать вопросы и получать ответы.\n\n"
    ...
    async def handle_keyboard_offline(self, message: Message):
        ...
        offline_description = (
            ...
            f"{create_premium_separator()}\n\n"
            f"«Главный герой» — это медиа-практикум с профессиональной съёмкой, где участники проходят путь от диалога к публичности. Вы не просто учитесь брать и давать интервью — вы становитесь героем собственного медиа-материала, работаете в кадре, получаете обратную связь и выходите с готовым контентом, который можно использовать дальше.\n\n"
            f"Этот формат создан для тех, кому важно говорить и быть услышанным\n\n"
            f"{create_premium_separator()}\n\n"
            f"<b>Предприниматель</b>\n"
            f"чтобы улучшить навыки нетворкинга и управления\n\n"
            f"<b>Спикер, коуч, психолог</b>\n"
            f"чтобы быстро и детально распаковывать людей и информацию\n\n"
            f"<b>Менеджер по продажам</b>\n"
            f"чтобы отточить искусство диалога и продавать больше\n\n"
            f"<b>Преподаватель</b>\n"
            f"чтобы использовать вопросы как инструмент для достижения лучших результатов у учеников\n\n"
            f"<b>Блогер/подкастер</b>\n"
            f"чтобы начать вести интервью или усовершенствовать навыки\n\n"
            f"<b>Журналист/продюсер</b>\n"
            f"чтобы войти в мир СМИ\n\n"
            f"<b>Эксперт</b>\n"
            f"чтобы активнее расти и развиваться через правильные вопросы\n\n"
            f"<b>Любой человек</b>\n"
            f"желающий сделать свои диалоги, а значит и жизнь, более насыщенными и интересными\n\n"
            f"{create_premium_separator()}\n\n"
            ...
            f"{create_premium_separator()}\n\n"
            f"Живой практикум «Главный герой» — превращаем теорию интервью в работающую систему. Проверено на 3000+ интервью.\n\n"
            f"Не забудьте после оплаты прислать свое имя в Телеграм на <a href='https://t.me/niktatv'>@niktatv</a>, чтобы вас включили в рабочую группу.\n\n"
            f"{create_premium_separator()}\n\n"
            f"<b>Почему этот формат работает. Вы:</b>\n\n"
            f"🎯 видите себя со стороны\n"
            ...
            f"И главное — вы выходите не только с навыком, но и с медиа-результатом.\n\n"
            f"{create_premium_separator()}\n\n"
            f"<b>Тарифы</b>\n\n"
            f"<b>СЛУШАТЕЛЬ</b>\n"
            f"Присутствие\n\n"
            ...
            f"Разрабатываю идеи, создаю текстовый, аудио- и видеоконтент с 2000 года как телеведущий, журналист, диктор, киносценарист и режиссёр. Провёл 3000+ интервью."
        )
