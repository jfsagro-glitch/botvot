...
def create_programs_tariff_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Онлайн · ВОПРОСЫ, КОТОРЫЕ МЕНЯЮТ ВСЁ (1-я ступень) →", url=online_url)],
        [InlineKeyboardButton(text="Офлайн · ГЛАВНЫЙ ГЕРОЙ →", url=offline_url)],
        [InlineKeyboardButton(text="Тарифы онлайн (1-я ступень) →", callback_data="sales:tariffs:online")],
        [InlineKeyboardButton(text="Тарифы онлайн (2-я ступень) →", callback_data="sales:tariffs:online:second")],
...
def create_lesson_keyboard_from_json(lesson_data: dict, user, general_group_id: str) -> InlineKeyboardMarkup:
    ...
    # ВАЖНО: Кнопка "Навигатор курса" (🧭) была удалена и больше не добавляется к клавиатуре заданий.
    # Навигатор доступен через постоянную клавиатуру внизу (кнопка 🧭).
    # Остальные кнопки также удалены, так как они теперь в постоянной клавиатуре внизу.
